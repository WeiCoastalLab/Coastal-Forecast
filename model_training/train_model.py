...
def split_dataset(dataset: pd.DataFrame, station_id: str, n_outputs: int) \
    ...
    target_scaled = scaler_target.fit_transform(data[:, -3:])
    with open(f'../model/{station_id}_scalers.pkl', 'wb') as scaler_f:
...
def to_supervised(train: np.array, n_inputs: int, n_outputs: int) -> (np.array, np.array):
    ...
    # print(f'Flattened data shape: {data_tn.shape}')
    train_x, train_y = [], []
    in_start = 0
    ...
    for _ in range(len(data_tn)):
        ...
        if out_end <= len(data_tn):
            ...
            train_y.append(data_tn[in_end:out_end, -3:])
        # move along one step
        in_start += 1
...
def build_model(train_x: np.array, train_y: np.array, n_inputs: int, n_outputs: int, station_id: str) -> Sequential:
    ...
    # print(f'train_x before reshape: {train_x.shape}')
    ...
    # print(f'train_x after reshape: {train_x.shape}')
    ...
    plot_model(model, show_shapes=True, show_layer_names=True, to_file=f'../model/{station_id}_model_plot.png')
    # quit()
    print("Fitting model...")
    ...
    # save history
    with open(f'../training_data/{station_id}_training_history', 'wb') as file_pi:
